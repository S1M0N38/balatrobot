...
local function queueaction(func, delay)
	List.pushleft(Middleware.queuedactions, { func = func, delay = delay or 0 })
end
...
local function c_update()
	...
	_events = {}
	if
		not List.isempty(Middleware.queuedactions)
		and (not Middleware.currentaction or (Middleware.currentaction and Middleware.currentaction.complete))
	then
		local _func_and_delay = List.popright(Middleware.queuedactions)
		Middleware.currentaction =
			Middleware.add_event_sequence({ { func = _func_and_delay.func, delay = _func_and_delay.delay } })
	end
...
function Middleware.c_play_hand()
	firewhenready(function()
		-- Check if there's an action in the queue for select_cards_from_hand
		if not List.isempty(BalatrobotAPI.q_select_cards_from_hand) then
			local _action_data = List.popright(BalatrobotAPI.q_select_cards_from_hand)
			local _action = _action_data[2][1]
			local _cards_to_play = _action_data[2][2]
			return true, _action, _cards_to_play
		else
			return false
		end
	end, function(_action, _cards_to_play)
...
function Middleware.c_select_blind()
	...
	firewhenready(function()
		if
			...
			and (G.GAME.blind_on_deck == "Small" or G.GAME.blind_on_deck == "Big" or G.GAME.blind_on_deck == "Boss")
		then
			-- Check if there's an action in the queue for skip_or_select_blind
			if not List.isempty(BalatrobotAPI.q_skip_or_select_blind) then
				local _action_data = List.popright(BalatrobotAPI.q_skip_or_select_blind)
				local _action = _action_data[2][1]
				return true, _action
			else
				return false
...
function Middleware.c_choose_booster_cards()
	if Middleware.choosingboostercards == true then
		return
	end
	...
	firewhenready(function()
		-- Check if there's an action in the queue for select_booster_action
		if not List.isempty(BalatrobotAPI.q_select_booster_action) then
			local _action_data = List.popright(BalatrobotAPI.q_select_booster_action)
			local _action = _action_data[2][1]
			local _card = _action_data[2][2]
			local _hand_cards = _action_data[2][3]
			return true, _action, _card, _hand_cards
		else
			return false
		end
	end, function(_action, _card, _hand_cards)
		...
		if G.GAME.pack_choices - 1 > 0 then
			queueaction(function()
				firewhenready(function()
					return Middleware.BUTTONS.SKIP_PACK ~= nil
						and Middleware.BUTTONS.SKIP_PACK.config.button == "skip_booster"
						and Middleware.choosingboostercards == false
						and G
						and G.pack_cards
...
function Middleware.c_shop()
	...
	local _b_reroll_shop = Middleware.BUTTONS.REROLL
		and Middleware.BUTTONS.REROLL.config
		and Middleware.BUTTONS.REROLL.config.button
	...
	firewhenready(function()
		-- Check if there's an action in the queue for select_shop_action
		if not List.isempty(BalatrobotAPI.q_select_shop_action) then
			local _action_data = List.popright(BalatrobotAPI.q_select_shop_action)
			local _action = _action_data[2][1]
			local _card = _action_data[2][2]
...
function Middleware.c_rearrange_hand()
	firewhenready(function()
		-- Check if there's an action in the queue for rearrange_hand
		if not List.isempty(BalatrobotAPI.q_rearrange_hand) then
			local _action_data = List.popright(BalatrobotAPI.q_rearrange_hand)
			local _action = _action_data[2][1]
			local _order = _action_data[2][2]
			return true, _action, _order
		else
			return false
		end
	end, function(_action, _order)
		Middleware.c_play_hand()

		if not _order or #_order ~= #G.hand.cards then
			return
		end
...
function Middleware.c_rearrange_consumables()
	firewhenready(function()
		-- Check if there's an action in the queue for rearrange_consumables
		if not List.isempty(BalatrobotAPI.q_rearrange_consumables) then
			local _action_data = List.popright(BalatrobotAPI.q_rearrange_consumables)
			local _action = _action_data[2][1]
			local _order = _action_data[2][2]
			return true, _action, _order
		else
		...
		if not _order or #_order ~= #G.consumables.cards then
			return
		end
...
function Middleware.c_use_or_sell_consumables()
	firewhenready(function()
		-- Check if there's an action in the queue for use_or_sell_consumables
		if not List.isempty(BalatrobotAPI.q_use_or_sell_consumables) then
			local _action_data = List.popright(BalatrobotAPI.q_use_or_sell_consumables)
			local _action = _action_data[2][1]
			local _cards = _action_data[2][2]
			return true, _action, _cards
		else
			return false
		end
	end, function(_action, _cards)
...
function Middleware.c_rearrange_jokers()
	firewhenready(function()
		-- Check if there's an action in the queue for rearrange_jokers
		if not List.isempty(BalatrobotAPI.q_rearrange_jokers) then
			local _action_data = List.popright(BalatrobotAPI.q_rearrange_jokers)
			local _action = _action_data[2][1]
			local _order = _action_data[2][2]
			return true, _action, _order
		else
		...
		if not _order or #_order ~= #G.jokers.cards then
			return
		end
...
function Middleware.c_sell_jokers()
	firewhenready(function()
		-- Check if there's an action in the queue for sell_jokers
		if not List.isempty(BalatrobotAPI.q_sell_jokers) then
			local _action_data = List.popright(BalatrobotAPI.q_sell_jokers)
			local _action = _action_data[2][1]
			local _cards = _action_data[2][2]
			return true, _action, _cards
		else
...
function Middleware.c_start_run()
	firewhenready(function()
		-- Check if there's an action in the queue for start_run
		if not List.isempty(BalatrobotAPI.q_start_run) then
			local _action_data = List.popright(BalatrobotAPI.q_start_run)
			local _action = _action_data[2][1]
			local _stake = _action_data[2][2]
			local _deck = _action_data[2][3]
			local _seed = _action_data[2][4]
			local _challenge = _action_data[2][5]
	...
	end, function(_action, _stake, _deck, _seed, _challenge)
		queueaction(function()
...
local function c_initgamehooks()
	...
	G.CONTROLLER.snap_to = Hook.addcallback(G.CONTROLLER.snap_to, function(...)
		...
		if
			_self
			and _self.snap_cursor_to.node
			and _self.snap_cursor_to.node.config
			and _self.snap_cursor_to.node.config.button
		then
	...
	G.FUNCS.can_skip_booster = Hook.addcallback(G.FUNCS.can_skip_booster, function(...)
		...
		if
			Middleware.BUTTONS.SKIP_PACK ~= nil
			and Middleware.BUTTONS.SKIP_PACK.config.button == "skip_booster"
			and Middleware.choosingboostercards == false
			and G
			and G.pack_cards
